...
for i in sorted(result.keys()):
    ...
    if result[i] == [0 for zero in xrange(8)]:
        ...
        continue
    find = False
    for j in sorted(combonations.keys()):
        if result[i] == combonations[j]:
            p.append('%s: %s' % (i, j))
            find = True
            break
    if find:
        ...
        continue
    for j in sorted(combonations.keys()):
        for k in sorted(combonations.keys()):
            tmp = map(lambda x, y: x or y, combonations[j], combonations[k])
            if result[i] == tmp:
    ...
    for j in sorted(combonations.keys()):
        for k in sorted(combonations.keys()):
            for l in sorted(combonations.keys()):
                tmp = map(lambda x, y, z: x or y or z, combonations[j], combonations[k], combonations[l])
                if result[i] == tmp:
    ...
    for j in sorted(combonations.keys()):
        for k in sorted(combonations.keys()):
            for l in sorted(combonations.keys()):
                for m in sorted(combonations.keys()):
                    tmp = map(lambda x, y, z, n: x or y or z or n, combonations[j], combonations[k], combonations[l], combonations[m])
                    if result[i] == tmp:
